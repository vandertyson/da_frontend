    ...
    <v-form v-model="valid" ref="form" lazy-validation>
      <v-container grid-list-xl fluid>
        ...
        <v-layout v-if="ready" row wrap>
          <v-flex md5>
            <v-card class="pa-4">
              <h4 class="headline mb-0">Thông tin chung hóa đơn bán hàng</h4>
              <v-autocomplete
      ...
      <v-dialog v-model="dialog" persistent max-width="290">
        <v-card>
          ...
          <v-card-actions>
            ...
            <router-link to="/invoice" tag="button">
    ...
    pAll
      .then(res => {
        ...
        if (this.$route.params.id) {
          HTTP.get(URL.getInvoiceById + this.$route.params.id)
            .then(response => {
  ...
  methods: {
    ...
    save: function() {
      ...
      if (this.$route.params.id) {
        ...
        HTTP.put(URL.updateInvoice, post_param)
          .then(response => {
            this.posts = response.data;
            this.$data.message = "AR-Invoice editted successfully!";
            this.$data.snackbar = true;
          })
          .catch(e => {
            console.log(e);
        ...
        console.log(post_param);
        HTTP.post(URL.addInvoice, post_param)
          .then(response => {
            this.posts = response.data;
            this.$data.message = "AR-Invoice added successfully!";
            this.$data.snackbar = true;
          })
          .catch(e => {
            console.log(e);
    ...
    onSelectCopy: function(event) {
      ...
      if (event["type"] == "delv") {
        HTTP.get(URL.getDeliveryById + event["id"])
          .then(response => {
            //doan nay gan lai cac truong cua cai delivery vao cac bien tren invoice
            console.log(response.data);
            this.$data.selectedCustomer = response.data.code;
            this.customnerSelect(response.data.code);
            this.$data.selected_contact_person = parseInt(
