  ...
  <div id="page-forms">
    ...
    <v-container v-if="ready" grid-list-xl fluid>
      <v-layout row wrap>
        ...
        <router-link v-if="!isSelect" to="/delivery/add" tag="button">
          <v-btn round color="success" class="text-lg-right">Add new Delivery</v-btn>
        </router-link>
...
export default {
  ...
  methods: {
    ...
    deleteDelivery: function(delvID) {
      if (confirm("Bạn có chăc chắn muốn xóa?")) {
        HTTP.delete(URL.deleteDelivery + delvID)
          .then(response => {
